...
def floats_to_pixels_standardized(x, bound=255):
    ...
    bound : int, optional
        The max positive value to which values are bounded (default: 255)    
...
def visual_grid_4d_rgb(X, padding=4, pixel_transform="normalized", dim=None):
    ...
    X: numpy.array
        A 4-dim numpy array where the third dimension has value 3
    padding: int, optional
        Number of pixel to pad between two images
    transform: String, "normalized"
        Type of transformation to apply to arrays values to bound them to [0,255].
        Possible values are "normalized" and "standardized"
...
def visual_grid_4d_multichannel(X, padding=4, pixel_transform="normalized"):
    """
    Transforms a 4-dim numpy array with shapes (height, width, channels, filtr),
    and with any number of channels, into a 3-dim grid of grayscale images (aka filters)
    with shapes (height, width, 1) that can be visualized in matplotlib.
    ...
    numpy.array
        A 3-d grid of grayscale images where values are bounded to [0,255]. 
    """
    (H, W, C, F) = X.shape
...
def visual_grid_3d_multichannel(X, padding=4, pixel_transform="normalized", dim=None):
    """
    Transforms a 3-dim numpy array with shapes (height, width, filter),
    into a 3-dim grid of grayscale images (aka filters)
    with shapes (height, width, 1) that can be visualized in matplotlib.
    Every element (height, width) is transformed into a separate image (height, width, 1).
    Values of the input array are bounded to [0,255].
